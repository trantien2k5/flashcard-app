...
- Tab Ôn tập ([js/features/review.js](js/features/review.js)): tối giản, chỉ 2 thẻ + 1 nút. Thẻ streak (`.streak-card`, `computeStreak()`) → thẻ số từ đến hạn (`.due-card`, `todaysReviewBatch()`, KHÔNG giới hạn số lượng) — bấm vào (`#dueCardHead`) mở/đóng danh sách từng từ đang đến hạn (`.due-list`/`.due-row`, biến module `reviewDueListOpen` giữ trạng thái mở/đóng qua các lần re-render) → nút `.review-cta` "Ôn tập" cuối trang, disabled khi không có từ nào đến hạn. Không có banner/mẹo/số liệu phụ nào khác. Streak/Thời gian học chi tiết vẫn thuộc tab Thống kê (`.stat-cards`).
