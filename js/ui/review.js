...
function startReviewCountdown(targetDate){
  ...
  const tick = ()=>{
    ...
    if(!el || !document.body.contains(el)){ stopReviewCountdown(); return; }
    const ms = targetDate - new Date();
    if(ms<=0){ renderReviewTab(); return; }
...
function renderReviewTab(){
  ...
  pageSub.textContent = due.length===0
    ? (hasNewCards ? 'Không có từ đến hạn — học thêm từ mới nhé' : 'Bạn đã ôn hết, hẹn gặp lại ngày mai!')
    : (remaining===0 ? 'Đã đạt giới hạn ôn tập hôm nay' : 'Ôn lại từ đến hạn bằng active recall');

  const byTopic = {};
  due.forEach(c=>{ byTopic[c.topicId]=(byTopic[c.topicId]||0)+1; });
  ...
  if(due.length===0){
    ...
    listHtml = `<div class="section-label">Theo chủ đề</div>` + TOPICS.filter(t=>byTopic[t.id]).map(t=>`
  ...
  if(startBtn) startBtn.addEventListener('click', ()=> startReviewSession(todaysReviewBatch()));
  const quickBtn = document.getElementById('quickReviewBtn');
  if(quickBtn) quickBtn.addEventListener('click', ()=> startReviewSession(shuffle(todaysReviewBatch()).slice(0,5)));
  const goLearnBtn = document.getElementById('goLearnBtn');
  if(goLearnBtn) goLearnBtn.addEventListener('click', ()=> switchTab('learn'));
  mainEl.querySelectorAll('.topic-row').forEach(el=>{
    el.addEventListener('click', ()=> startReviewSession(todaysReviewBatch(el.dataset.topic)));
  });
