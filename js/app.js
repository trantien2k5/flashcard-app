...
function refreshChrome(){ streakChip.textContent = `🔥 ${computeStreak()}`; }
...
document.getElementById('libraryBtn').addEventListener('click', ()=> switchTab('library'));
...
async function init(){
  const [p, rl, rd, nl, rt, s] = await Promise.all([
    storeGet('progress'), storeGet('reviewLog'), storeGet('reviewsDoneLog'), storeGet('newWordsLog'), storeGet('ratingLog'), storeGet('settings')
  ]);
  if(p) progress = p;
  if(rl) reviewLog = rl;
